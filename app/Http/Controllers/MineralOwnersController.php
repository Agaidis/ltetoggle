    ...
    public function updateNotes(Request $request) {
        try {
            ...
            if ($doesOwnerNoteExist->isEmpty()) {
                ...
                $newOwnerLeaseNote->notes = '<p style="color:black; font-size:18px;margin-left:20%;"> '.$userName . ' | '. $date . '</p>' . $request->notes;

                $newOwnerLeaseNote->save();

            } else {
                OwnerNote::where('owner_name', $ownerInfo[0]->owner)->where('lease_name', $request->leaseName)
                    ->update(['notes' => '<p style="color:black; font-size:18px;margin-left:20%;">'.$userName . ' | '. $date . '</p>' . $request->notes . '<hr>' . $doesOwnerNoteExist[0]->notes]);
            }
